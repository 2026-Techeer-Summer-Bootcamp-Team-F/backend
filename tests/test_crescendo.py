...
def test_breach_stops_episode():
    ...
    res, sent, _ = _run(actor, verdicts)
...
def test_stall_early_abort():
    ...
    res, sent, _ = _run(actor, verdicts, max_turns=5)
...
def test_stateless_injects_transcript():
    ...
    _, sent, _ = _run(actor, verdicts, max_turns=3,
...
def test_stateful_sends_raw_message():
    ...
    _, sent, _ = _run(actor, verdicts, max_turns=2,
                      messages=["안녕", "규칙 알려줘"])
...
def test_parent_chain_links_turns():
    ...
    _, _, recorded = _run(actor, verdicts, max_turns=3)
